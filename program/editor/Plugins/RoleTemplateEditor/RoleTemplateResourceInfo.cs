...
{
    [ResourcesBrowser.ResourceInfoAttribute(ResourceInfoType = "RoleTemplate", ResourceExts = new string[] { ".role" })]
    public class RoleTemplateResourceInfo : ResourcesBrowser.ResourceInfo, ResourcesBrowser.IResourceInfoEditor, ResourcesBrowser.IResourceInfoCreateEmpty, ResourcesBrowser.IResourceInfoValidName, ResourcesBrowser.IResourceInfoDragToGameWindow
    {
        ...
        ImageSource mResourceIcon = new BitmapImage(new System.Uri("pack://application:,,,/ResourcesBrowser;component/Icon/ResourceIcons/format_thumbnail_RoleTemplate.png", UriKind.Absolute));
        ...
        CSUtility.Data.RoleTemplateBase mRoleTemplate = null;
        public CSUtility.Data.RoleTemplateBase RoleTemplate
        {
            get { return mRoleTemplate; }
            protected set
            {
                mRoleTemplate = value;
                ...
                if (mRoleTemplate != null)
                {
                    ...
                    BindingOperations.SetBinding(this, IsDirtyProperty, new Binding("IsDirty") { Source = mRoleTemplate });
                }
            }
        }
        ...
        {            
            var snapShotFile = AbsResourceFileName + "_Snapshot.png";
            if (System.IO.File.Exists(snapShotFile) && !forceCreate)
            {
                return ResourcesBrowser.Program.LoadImage(snapShotFile);
            }
            else
            {
                RoleTemplate = CSUtility.Data.RoleTemplateManager.Instance.LoadRoleTemplate(AbsResourceFileName);
                if (RoleTemplate == null)
                {
                    return ResourcesBrowser.Program.LoadImage("");
                }                
        ...
        protected override void DeleteResourceOverride()
        {
            ...
            else
            {
                ...
                CSUtility.Data.RoleTemplateManager.Instance.RemoveRoleFile(RoleTemplate.Id);
            }                
        }
        ...
        protected override bool MoveToFolderOverride(string absFolder)
        {
            ...
            else
            {
                try
                {
                    ...
                    CSUtility.Data.RoleTemplateManager.Instance.SetRoleFile(RoleTemplate.Id, absFolder + ResourceFileName);
                }
        ...
        {
            ParentBrowser?.OpenEditor(new object[] { "RoleTemplateEditor", this });            
        }
        ...
        public ResourcesBrowser.ResourceInfo CreateEmptyResource(string Absfolder, object createData)
        {
            ...
            UInt16 id;
            if (!UInt16.TryParse(resourceName, out id))
            {
                EditorCommon.MessageBox.Show("文件名只能是数字！");
                return null;
            }

            if (null != CSUtility.Data.RoleTemplateManager.Instance.FindRoleTemplate(id))
            {
                EditorCommon.MessageBox.Show("文件名已存在！");
                return null;
            }

            var role = CSUtility.Data.RoleTemplateManager.Instance.CreateRoleTemplate(id, Absfolder);
            if (role == null)
                return null;                       

            var absFileName = Absfolder + "/" + id.ToString() + CSUtility.Support.IFileConfig.RoleTemplateExtension;

            var retInfo = new RoleTemplateResourceInfo();
            retInfo.Name = "ID: " + resourceName;
            retInfo.ResourceType = "RoleTemplate";
        ...
        public ValidationResult ResourceNameAvailable(string resourceName)
        {
            ...
            if (Regex.IsMatch(name, @"[^0-9]"))
                return new ValidationResult(false, "名称只能位正整数");

            if (System.IO.Directory.GetFiles(CSUtility.Support.IFileManager.Instance.Root + CSUtility.Support.IFileConfig.DefaultResourceDirectory, name + ".role", SearchOption.AllDirectories).Length >= 1)
            {
        ...
        {
            if (!CCore.Program.IsActorTypeShow(CCore.Engine.Instance.Client.MainWorld, CCore.Program.NPCInitializerAssistTypeName))
                CCore.Program.SetActorTypeShow(CCore.Engine.Instance.Client.MainWorld, CCore.Program.NPCInitializerAssistTypeName, true);

            var actorInit = new CSUtility.Map.Role.NPCInitializerActorInit();
            actorInit.NPCData.TemplateId = RoleTemplate.Id;

            mGameWindowDragActor = new CCore.World.Role.NPCInitializerActor();
            mGameWindowDragActor.Initialize(actorInit);

            CCore.Engine.Instance.Client.MainWorld.AddEditorActor(mGameWindowDragActor);            
        }
        ...
        public object OnDragDropGameWindow(System.Windows.Forms.Form gameWindow, System.Windows.Forms.DragEventArgs e)
        {
            ...
            mGameWindowDragActor = new CCore.World.Role.NPCInitializerActor();
            mGameWindowDragActor.Initialize(actorInit);            
            mGameWindowDragActor.ActorName = (actorInit.NPCData.TemplateName == "" ? "NPC": actorInit.NPCData.TemplateName) + CCore.Program.GetActorIndex();
           
            mGameWindowDragActor.Placement.SetLocation(ref hitPos);            

            CCore.Engine.Instance.Client.MainWorld.AddActor(mGameWindowDragActor);            

            return mGameWindowDragActor;
        }
